...
export default {
  ...
  name: "students-table",
  computed: {
    ...mapState(['strings', 'sheets', 'userID', 'typeControlCredit', 'typeControlExamine']),
    isThemedSheet() {
  ...
  methods: {
    convertGrade(grade, gradeType = "semester", gradeSemester = 0) { // 000000003 - zachet
      ...
      if (gradeType === "semester") {
        let isGradeTypeOfControl = (this.sheet.type_control_code === this.typeControlExamine || this.sheet.type_control_code === this.typeControlCredit);

        let maximumGrade = (isGradeTypeOfControl ? 100 : 70);
    ...
    updateGradeAbsence(e, grade, student, point_semester = null, point_control_event = null) {
      const GUIDGrade = e.target.checked ? this.absenceGUID : this.emptyGUID;
      point_control_event = 0;
      this.updateGrade(
          GUIDGrade,
          student, this.sheet.guid,
          '',
          point_semester,
          point_control_event,
          'insert_absence'
    ...
    async updateGrade(grade, student, sheet, theme = '', point_semester = null, point_control_event = null, apiUsage = 'insert_grade') {
      ...
      if (result.success) {
        ...
        //console.log(this.sheet.students[0]);
        this.$store.commit('updateSheet', this.sheet);
      }
    },
...
<!--                <div class="row" v-if="!showBRS.includes(sheet.level_education.replaceAll(' ', ''))">-->
                <div class="row" v-if="!showBRS && isAgreedAllow">
                  <div class="col-12">
                ...
                <template v-else>
                  ...
                  <div class="row pt-2">
                    <div class="col-6">
                      ...
                      <input type="checkbox" class=""
                             :checked="studentGrade.grade === absenceGUID"
                  ...
                  <div class="row pt-2">
                    <div class="col-12 d-flex align-items-center">
                      Текущая отметка: <b class="ml-2">{{ renderGrade(studentGrade.grade) }}</b>
                    </div>
                  </div>
                </template>
              </div>
