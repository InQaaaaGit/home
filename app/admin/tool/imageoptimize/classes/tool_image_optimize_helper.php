...
use stdClass;
use tool_imageoptimize\factory\OptimizePDFFactory;
...
class tool_image_optimize_helper extends \tool_image_optimize
{
    ...
    public function get_enabled_mimetypes(): array
    {
        ...
        ///$this->enabledmimetypes[] = 'application/pdf'; // TODO improve?
    ...
    public function get_files_to_process($maxchunksize = 2000): array
    {
        ...
        foreach ($rows as $row) {
            ...
            mtrace('files = ' . $row->fileid);
        }
        #mtrace('files = ' . count($fileids));
        if (empty($fileids)) {
            return [];
        }

        $fileids = array_values($fileids);
        $max_file_size = get_config('tool_imageoptimize', 'pdf_max_file_size');
        $where_clause_pdf = '';
        if (!empty($max_file_size)) {
            $where_clause_pdf = " AND CASE WHEN mimetype = 'application/pdf' THEN filesize>$max_file_size ELSE TRUE END ";
        }
        list($sqlin, $inparms) = $DB->get_in_or_equal($fileids);
        $sql = "SELECT * from {files} WHERE id " . $sqlin . $where_clause_pdf;
     ...
     */
    public function process_files(int $chunksize = 2000): void
    {
        global $DB;

        $filestoprocess = $this->get_files_to_process($chunksize);
        mtrace("file to process = " . count($filestoprocess));
    ...
    protected function process_file($file): bool
    {
        ...
        $tofilepath = $this->get_temp_file_path($file);
        ...
        if ($file->mimetype === 'application/pdf' ) {
            $optimizerchain = (new OptimizePDFFactory())->createOptimizer();
        } else {
            $optimizerchain = OptimizerChainFactory::create();
            $optimizerchain->setTimeout(320);
        }
    ...
    Old vs. New Filesize: " . $fileold->filesize . " vs " . $file->filesize
        ...
        if (!empty($this->dryrun)) {
            ...
            $this->cleanup_working_directories($file->contenthash, $fromfilepath, $tofilepath);
            // Delete record from tool_imageoptimize_files table.
    ...
    private function get_temp_file_path($file): string
    {
        ...
        return $CFG->tempdir . '/' . random_string() . '.' . $this->get_file_extension($file);
    }
    ...
    /**
     * Handles the task execution call.
     */
    ...
    /**
     * Handles the cli execution call.
     * @param array $options
     */
            ...
            foreach ($records as $record) {
                ...
                if (!$DB->record_exists('tool_imageoptimize_files', $conditions)) {
                    try {
                       ...
                       // $insertobject // if duplicate index fired
                    }
                }
