...
try {
    ...
    $id_trainingcertificate = required_param('id_trainingcertificate', PARAM_TEXT);
    $user_id = optional_param('user_id', '',PARAM_TEXT);

    if (empty($id_trainingcertificate)) {
        throw new Exception('Отсутствует обязательный параметр: id_trainingcertificate');
    }
    ...
    $options->set_properties([
        ...
        'id_trainingcertificate' => $id_trainingcertificate
    ]);

    $result = di::get_instance()
        ->get_request('download_order')
        ->request($options)
    ...
    header('Content-Disposition: attachment; filename="Справка.pdf"');    */
    header('Content-Type: application/vnd.openxmlformats-officedocument.wordprocessingml.document');
    header('Content-Disposition: attachment; filename="Справка.zip"');
    ob_clean();
    echo $result;
    exit;
    ...
    $PAGE->set_url('/local/cdo_order_documents/download.php');
    ...
    $errorurl = new moodle_url('/local/cdo_order_documents/error.php', [
        'code' => $errorcode,
