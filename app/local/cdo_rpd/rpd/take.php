...
//require_once($CFG->dirroot.'/ulsu/rpd/tcpdf.php');
...
require_once($CFG->dirroot.'/ulsu/rpd/cdolib.php');
...
$yummy = array(
    ...
    "тесты",
    "эссе",
	"Вопросы к Экзамену"  //lemuria 10.10.2024
);
...
if(!empty($_GET['user_id'])){
	...
	$_REQUEST['user_id'] = $_GET['user_id'] = $_POST['user_id'] =  $user_id_rpd;
...
$serviceRPD = new local_cdo_rpd_external_own();

$serviceRPD->USER = $user_info;
...
//lemuria 26.09.2024
if(!$is_ochka_form && !empty($plan_zaochka)){
...
//var_dump($plan["Дисциплины"][$rpd->info->discipline]);
...
if(!empty($pdisc_zaoch["Записи"])){
	...
	foreach ($pdisc_zaoch["Записи"] as $pp) {
...
/*
var_dump($courses);
var_dump($courses_zaoch);

var_dump($courses_zaoch_ochka);
...
foreach ($rpd->info->developers->mainDeveloper as $dev) {
	...
	//var_dump(json_decode($serviceULSU->GetUserInfo(['id' => $dev->id])),true);
	
	//var_dump($pooos);
	//var_dump($serviceULSU->GetUserInfo(['id' => $dev->id]));
	//exit('dfdf');
   ...
   // $devssign .= "{$dev->user}<br>";
	...
	foreach($pooos['positions'] as $pos){
		...
		if($count_p > 1){
			if($count_p > 1  && $first){
			   $developers .= "<tr><td {$count_p}>{$name_dev}</td><td>{$pos['department']}</td><td>{$pos['name']},{$f}</td></tr>";
			}
			elseif(!$first){
				$developers .= "<tr><td>{$pos['department']}</td><td>{$pos['name']},{$f}</td></tr>";

			}
		
		}
		else{
			$developers .= "<tr><td>{$name_dev}</td><td>{$pos['department']}</td><td>{$pos['name']},{$f}</td></tr>";
		}
...
//28  В аннотации п6. Контроль успеваемости отсутствуют виды текущего контроля. Например, в моей РПД должны быть тесты.
...
$data = [
    ...
    'COURSE' => $course_title,   //lemuria 11.10.2024
...
//взять архив согласования.
    ...
    //$url_string = $CFG->wwwroot.'/ulsu/rpd/take.php?type=annotation&user_id='.$USER->id.'&subtype=zip&rpd_id='.$_GET['rpd_id'].'&discipline='.$_GET['discipline'].'&edu_plan='.$_GET['edu_plan'];
	
	$url_string = $CFG->wwwroot.'/ulsu/rpd/take.php?type=annotation&user_id='.$_REQUEST['user_id'].'&subtype=zip&rpd_id='.$_GET['rpd_id'].'&discipline='.$_GET['discipline'].'&edu_plan='.$_GET['edu_plan'];
    ...
    //$url_string = $CFG->wwwroot.'/ulsu/rpd/take.php?type=rpd&user_id='.$USER->id.'&subtype=zip&rpd_id='.$_GET['rpd_id'].'&discipline='.$_GET['discipline'].'&edu_plan='.$_GET['edu_plan'];
	
	$url_string = $CFG->wwwroot.'/ulsu/rpd/take.php?type=rpd&user_id='.$_REQUEST['user_id'].'&subtype=zip&rpd_id='.$_GET['rpd_id'].'&discipline='.$_GET['discipline'].'&edu_plan='.$_GET['edu_plan'];
    ...
    //$url_string = $CFG->wwwroot.'/ulsu/rpd/take.php?type=fos&user_id='.$USER->id.'&subtype=zip&rpd_id='.$_GET['rpd_id'].'&discipline='.$_GET['discipline'].'&edu_plan='.$_GET['edu_plan'];
	
	$url_string = $CFG->wwwroot.'/ulsu/rpd/take.php?type=fos&user_id='.$_REQUEST['user_id'].'&subtype=zip&rpd_id='.$_GET['rpd_id'].'&discipline='.$_GET['discipline'].'&edu_plan='.$_GET['edu_plan'];
...
function fn_ar_gg_sort_detailed_competences($comps){
    ...
    foreach ($comps as $key__s => $numberss) {
		...
		//lemuria лютый вопрос что делать дальше
		$short_code_comp = $numberss->short_code;
		
		//lemuria 22.09.2024
		if(!empty($short_code_comp)){
				$current_number_comps = explode('-', $short_code_comp);
...
//another rpd is existQuestion
    ...
    foreach($rpd->questionsForAllThemes as $questions) {
		...
		//var_dump($t1);
				...
				$t1 = str_replace('</ol>', '', $t1); // откуда то берется <ol> тег и все ломает
			    $t1 = str_replace('</li>', '', $t1); // откуда то берется <ol> тег и все ломает
				//var_dump('sdfsdfsdf3343434');var_dump('sdfsdfsdf3343434');
				//var_dump($t1);
...
EOT;
        ...
        if ($questions->code == 'credit_question') {
            ...
            if ($t1 != "") {
				...
				$t1 = str_replace('<ul>', '', $t1); // откуда то берется <ol> тег и все ломает
...
EOT;
        ...
        if ($questions->code == 'questions_obsugd') {
            ...
            if ($t1 != "") {
				...
				<h3>4.%INDEX%. Вопросы для обуждения</h3>
				<table>
					<thead>
						<tr style="font-weight: bold; background-color: lightgray;">
							<th width="20%">Индекс компетенции</th>
...
EOT;
    ...
    $type_names = [
        ...
        'exam_assign' => "Задачи к экзамену",   //lemuria 16.09.2024
...
EOT;
	...
	//exit('sdfsdfsdfsdfsdf');
	
    foreach ($rpd->forms as $form) {

        $is_zaochka = false;
		
		 $is_zaochka_ochka = false;
		...
		if($form->name == 'очно-заочная'){
			
			 $is_zaochka_ochka = true;
		}
		...
		//var_dump($disc2sem[$rpd->info->discipline]);
...
//$dsems['cur_contr'] = implode("<td></td>",$modified_cur_contr);
...
$data['global_lib'] .='<br>
	...
	<p>1.1. Цифровой образовательный ресурс IPRsmart : электронно-библиотечная система : сайт / ООО Компания «Ай Пи Ар Медиа». - Саратов, [2023]. – URL: http://www.iprbookshop.ru. – Режим доступа: для зарегистрир. пользователей. - Текст : электронный.</p>
	<p>1.2. Образовательная платформа ЮРАЙТ : образовательный ресурс, электронная библиотека : сайт / ООО Электронное издательство «ЮРАЙТ». – Москва, [2023]. - URL: https://urait.ru. – Режим доступа: для зарегистрир. пользователей. - Текст : электронный.</p>
	<p>1.3. База данных «Электронная библиотека технического ВУЗа (ЭБС «Консультант студента») : электронно-библиотечная система : сайт / ООО «Политехресурс». – Москва, [2023]. – URL: https://www.studentlibrary.ru/cgi-bin/mb4x. – Режим доступа: для зарегистрир. пользователей. – Текст : электронный.</p>
	<p>1.4. Консультант врача. Электронная медицинская библиотека : база данных : сайт / ООО «Высшая школа организации и управления здравоохранением-Комплексный медицинский консалтинг». – Москва, [2023]. – URL: https://www.rosmedlib.ru. – Режим доступа: для зарегистрир. пользователей. – Текст : электронный.</p>
	<p>1.5. Большая медицинская библиотека :  электронно-библиотечная система : сайт / ООО «Букап». – Томск, [2023]. – URL: https://www.books-up.ru/ru/library/ . – Режим доступа: для зарегистрир. пользователей. – Текст : электронный.</p>
	<p>1.6. ЭБС Лань : электронно-библиотечная система : сайт / ООО ЭБС «Лань». – Санкт-Петербург, [2023]. – URL:  https://e.lanbook.com. – Режим доступа: для зарегистрир. пользователей. – Текст : электронный.</p>
	<p>1.7. ЭБС Znanium.com : электронно-библиотечная система : сайт / ООО «Знаниум». - Москва, [2023]. - URL:  http://znanium.com . – Режим доступа : для зарегистрир. пользователей. - Текст : электронный.</p>
	<p><b>2. КонсультантПлюс [Электронный ресурс]: справочная правовая система. / ООО «Консультант Плюс» - Электрон. дан. - Москва : КонсультантПлюс, [2023].</b></p>
	<p><b>3. Базы данных периодических изданий:</b></p>
	<p>3.1. eLIBRARY.RU: научная электронная библиотека : сайт  / ООО «Научная Электронная Библиотека». – Москва, [2023]. – URL: http://elibrary.ru. – Режим доступа : для авториз. пользователей. – Текст : электронный</p>
	<p>3.2. Электронная библиотека «Издательского дома «Гребенников» (Grebinnikon) : электронная библиотека / ООО ИД «Гребенников». – Москва, [2023]. – URL:  https://id2.action-media.ru/Personal/Products. – Режим доступа : для авториз. пользователей. – Текст : электронный.</p>
	<p><b>4. Федеральная государственная информационная система «Национальная электронная библиотека» : электронная библиотека : сайт / ФГБУ РГБ. – Москва, [2023]. – URL: https://нэб.рф. – Режим доступа : для пользователей научной библиотеки. – Текст : электронный.</b></p>
	<p><b>5. Российское образование : федеральный портал / учредитель ФГАУ «ФИЦТО». – URL: http://www.edu.ru. – Текст : электронный.</b></p>
	<p><b>6. Электронная библиотечная система УлГУ : модуль «Электронная библиотека»  АБИС Мега-ПРО / ООО «Дата Экспресс». – URL: http://lib.ulsu.ru/MegaPro/Web. – Режим доступа : для пользователей научной библиотеки. – Текст : электронный.</b></p>
</div>';
...
if(!empty($_GET['subtype']) && $_GET['subtype'] =='zip'){
    ...
    $file_name_for_save = $CFG->dirroot.'/ulsu/rpd/'.$_GET['type'].'.pdf';
