...
$PAGE->set_url(new moodle_url('/local/cdo_unti2035bas/send_practice.php', ['lrid' => $lrid]));
$title = get_string('sendpracticetitle', 'local_cdo_unti2035bas');
...
if (!$stream_params) {
    ...
    echo $output->render_from_template('local_cdo_unti2035bas/send_practice', $template_data);
    echo $output->footer();
    exit;
}
...
foreach ($users as $userid) {
    ...
    if (!empty($unti_id)) {
        try {
            $result = $handler->send_grade_to_practice_with_success_status(
                $unti_id,
                $lrid,
                $flow_id,
                $parent_course_id
            ...
            if ($result['success']) {
                ...
                $user_data['send_message'] = 'Успешно отправлено';
