...
export default {
  ...
  methods: {
    ...
    writeGradesFromNumbers(number1, number2) {
    ...
    async updateGrade(grade, student, sheet, theme = '', point_semester = null, point_control_event = null, apiUsage = 'insert_grade') {
      ...
      if (point_semester != null) {
        if (point_semester !== '') {
           point_semester1 = this.convertGrade(point_semester, "semester");
        }

        this.$store.commit('updateSheet', this.sheet);
        Object.assign(parameters, {point_semester: point_semester1});
      }
      if (point_control_event != null) {
        if (point_control_event !== '') {
           point_control_event1 = this.convertGrade(point_control_event, "point_control", point_semester);
        }
        this.$store.commit('updateSheet', this.sheet);
        Object.assign(parameters, {point_control_event: point_control_event1});
      }
                  ...
                  <div class="row pt-2">
                    <div class="col-6">
                      ...
                      <input type="checkbox" class=""
                             :checked="absenceGUID === studentGrade.grade || studentGrade.grade===''"
                  ...
                  <template v-if="studentGrade.point_semester !== -1">
                    ...
                    <div class="row pt-2">
                      <div class="col-12 d-flex align-items-center">
                        ...
                        <b class="ml-2" v-else>{{ writeGradesFromNumbers(studentGrade.point_semester,studentGrade.point_control_event) }}</b>
                      </div>
                    </div>
                  </template>
                </template>
